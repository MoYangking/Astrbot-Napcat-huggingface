...
if [ -x /home/user/QQ.AppImage ]; then
  exec dbus-launch $PROXY_PREFIX /home/user/QQ.AppImage --appimage-extract-and-run --no-sandbox --disable-gpu --disable-software-rasterizer ${NAPCAT_FLAGS:-}
fi

# Fallback to extracted AppRun
exec dbus-launch $PROXY_PREFIX /home/user/napcat/AppRun --no-sandbox --disable-gpu --disable-software-rasterizer ${NAPCAT_FLAGS:-}
